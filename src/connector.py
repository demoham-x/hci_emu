        ...
        try:
            ...
            while True:
                ...
                if not self.connected_device:
                    ...
                    print(f"\n[BURST READ] Device disconnected after {read_count} reads")
                    break

                try:
                    target_read = read_count + 1
                    print(f"[BURST READ] Starting read #{target_read}")
                    self._burst_read_inflight = asyncio.create_task(
                        self.gatt_client.read_value(handle)
                    )
                    value = await asyncio.wait_for(self._burst_read_inflight, timeout=5.0)

                    if value is not None:
                        read_count += 1
                        print(f"[BURST READ] Read #{read_count}")
                        ...
                        if count > 0 and read_count >= count:
                            ...
                            print(f"\n[BURST READ] Completed: {read_count} reads performed")
                            break
                    else:
                        logger.warning(f"Read failed on burst read #{read_count + 1}")
                        print(f"[BURST READ] Read #{read_count + 1} failed")

                    await asyncio.sleep(interval_sec)

                except asyncio.TimeoutError:
                    logger.warning(f"Burst read timeout on read #{read_count + 1}")
                    print(f"[BURST READ] Read #{read_count + 1} timed out")
                    ...
                    print(f"\n[BURST READ] Error on read {read_count + 1}: {e}")
                    await asyncio.sleep(interval_sec)
                finally:
                    if self._burst_read_inflight and self._burst_read_inflight.done():
                        self._burst_read_inflight = None

        except asyncio.CancelledError:
            logger.info(f"Burst read cancelled after {read_count} reads")
            print(f"\n[BURST READ] Cancelled after {read_count} reads")
        except Exception as e:
            logger.error(f"Burst read error: {e}")
            print(f"\n[BURST READ] Error: {e}")
        finally:
            print(f"[BURST READ] Total reads performed: {read_count}")
